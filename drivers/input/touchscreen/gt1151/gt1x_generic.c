...
{
	GTP_DEBUG("Clock ticks for an esd cycle: %d", msecs_to_jiffies(esd_work_cycle));
...
void gt1x_esd_switch(s32 on)
{
	...
	if (SWITCH_ON == on) {	/* switch on esd check */
		if (!esd_running) {
			...
			queue_delayed_work(gt1x_workqueue, &esd_check_work, msecs_to_jiffies(esd_work_cycle));
		}
...
static int charger_work_cycle = 2000;
...
{
	GTP_DEBUG("Clock ticks for an charger cycle: %d", msecs_to_jiffies(charger_work_cycle));
...
void gt1x_charger_switch(s32 on)
{
	...
	if (SWITCH_ON == on) {
		if (!charger_running) {
			...
			queue_delayed_work(gt1x_workqueue, &charger_switch_work, msecs_to_jiffies(charger_work_cycle));
		} else {
			spin_unlock(&charger_lock);
		}
	} else {
